...
class Scoreboard():
	def __init__(self, sw_settings, screen, game_stats):
		self.screen = screen
	...
	def prep_level(self):
		self.level_image = self.font.render(str(self.game_stats.level), True, self.text_color)
	...
	def prep_ships(self):
		...
		for ship_number in range(self.game_stats.ships_left):
			ship = Ship(self.sw_settings, self.screen)
